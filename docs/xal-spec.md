...
and arbitrary key/value entries as a two-cell tag band. The band is enabled
when the page frame has a non-empty `title`, a child-frame content `version`,
or a direct `<metadata>` child. Existing identified frames that have only an
`id` remain visually unchanged. Once the band is enabled, non-empty built-ins
are emitted in stable `id`, `title`, `version` order, followed by `<entry>`
...
most once. This context is distinct from document-level `<xaligo><metadata>`.
It contains only empty `<entry>` children; every entry requires non-empty
`key` and `value` attributes. Duplicate keys are retained.
...
Colors use `#RRGGBB` or `transparent`. Fixed widths use no-wrap shrink-to-fit
with clipping as the final overflow guard; omit `width`/`key-width` for auto
sizing. Tags preserve input order and pack greedily left-to-right against the
usable width `frame.width - 2 * row-gap`. `break-before="true"` forces a row
boundary before that entry.
...
Templates and icon SVGs for these tags are part of the xaligo core package
(`etc/resources/aws/templates/` and `etc/resources/aws/svg/Architecture-Group-Icons/`
in the core repository), not files inside this extension.
