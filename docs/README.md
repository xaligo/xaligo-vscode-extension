...
- [xal-spec.md](xal-spec.md) — the `.xal` DSL language specification: the V1
  compatibility profile, native V2 scene profile, layout primitives, AWS
  group tags, connection syntax, spacing classes, and validation rules.
...
These docs describe the `.xal` DSL and CLI behavior implemented by the stable
xaligo 0.2.1 release bundled with this extension (see this package's own
`package.json` for the exact pinned version). File paths mentioned inside them
(e.g. `etc/resources/aws/...`) refer to the xaligo core repository and its
published package, not to files inside this VS Code extension. If you also have
the [xaligo core repository](https://github.com/xaligo/xaligo) checked out, its
`docs/src/**` and `.github/instructions/*.instructions.md` are the actively
maintained originals and take precedence over these bundled copies.
